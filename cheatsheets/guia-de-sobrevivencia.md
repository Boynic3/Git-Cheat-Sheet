...
### ⚙️ Gerenciar Branches

- `git branch`: Lista as branchs atuais.
- `git switch -c <nome>` ou `git checkout -b <nome>`: Cria uma branch nova com o nome escrito em <nome>.
- `git switch <nome>` ou `git checkout <nome>`: Troca a branch atual.
- `git branch -d <nome>` e `git branch -D <nome>`: Deleta a branch escrita no <nome>.
